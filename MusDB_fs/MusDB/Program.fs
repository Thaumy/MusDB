...
//Eq predicate
let p a b = if a.Name=b.Name && a.Sha256=b.Sha256 && a.Type=b.Type then true else false
let hashp a b = if a.Sha256=b.Sha256 then true else false
...
let _ =
    if musicDbOnly.Length <> 0 then
        CLI.InColor ConsoleColor.Red (fun _ ->  CLI.Line "\n发现已登记曲目缺失 :")
        map (fun x -> CLI.Line x.Name) musicDbOnly |> ignore
    else
